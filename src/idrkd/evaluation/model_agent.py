...
def tool_selection_prompt(*, prompt: str, tools: list[dict[str, Any]]) -> str:
    return (
        ...
        "- blast radius or downstream impact -> impact_analysis, even when the only scoped ID is entity_id\n"
        "- create or produce a reconciliation recommendation -> reconcile\n"
        "- persist final reconciliation decision -> resolve_conflict\n"
        "- centroid drift -> get_centroid_drift\n"
        "- fetch or inspect an existing conflict state without a recommendation -> get_conflict\n\n"
        "Argument rules:\n"
        "- Copy argument values exactly from the task scope JSON when present.\n"
        "- Choose the tool from the user task meaning first; do not choose get_entity merely because entity_id is present.\n"
