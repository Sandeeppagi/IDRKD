...
During retrieval, the executable RAG orchestrator combines graph and vector candidates with RRF and optional MiniLM reranking. The critic splits the answer into atomic sentence-level claims, scores each claim independently, and uses the minimum claim score as the acceptance value. When a claim fails, the bounded second round retrieves a targeted query containing the original question and weakest claim. Intent-specific routing and unsupported-span presentation remain outside this legacy RAG path.
...
### 5.8.2 Query State Machine and Bounded Verification The historical measured RAG path maintains a typed `QueryState` and runs a fixed sequence of router marker, vector retrieval, graph traversal, RRF, optional reranking, synthesis, and critique. A maximum of two rounds bounds execution. A separate implemented LangGraph–AutoGen C3 workflow classifies intent, decomposes multi-hop requests, fans retrieval branches out concurrently, and delegates conflict reconciliation over A2A. No live C3 comparison is archived, so that implementation is not treated as measured evidence here.

The current implementation uses `cross-encoder/nli-deberta-v3-large` to score each atomic answer claim against concatenated evidence at threshold 0.78. When a claim fails, the weakest claim is included in a targeted follow-up retrieval query. The procedure returns after acceptance or after the bounded second round. The committed historical release artifact predates these changes and contains only whole-answer scores, so it cannot serve as evidence that the atomic path ran.
...
The training implementation supports 4-bit QLoRA, LoRA rank 16, alpha 32, dropout 0.05, and Phi-specific fused projection targets. The candidate artifact was produced from a split-v2 DPO adapter and quantised with llm-compressor AWQ using W4A16 asymmetric groups of 128. The committed generic adapter summaries concern a separate 500-record run, so epoch, step, and learning-rate claims for the split-v2 adapter are not reported as reproducible until its exact SFT/DPO summaries and datasets are published.
...
The measured candidate supports a narrower conclusion. The quantised Phi-4-mini checkpoint achieved 1.00 tool F1 and argument accuracy on an 89-case internal prompt-group holdout, p95 time to first token of 0.0332 seconds, and p95 completion latency of 1.3201 seconds. Five historical live repository answers achieved minimum whole-answer NLI faithfulness of 0.8744 and mean 0.9577, while the security suites passed. However, the corrected record rejects promotion because recall@10 is 0.2833 and atomic-claim evidence is absent. The results support internal tool-call competence and local serving feasibility, but not release-quality RAG. C1 discrimination, the complete C2 teacher/BFCL/VRAM criteria, and C3 multi-agent superiority remain unproven because their required comparative evidence was not executed.

## 7.2 Recommendations

- Evaluation should report tool-call conformance, protocol execution, semantic backend outcome, and answer quality separately. Official BFCL and independently authored tasks should be added before claiming generalisation. Teacher traces should retain provider/version/request provenance and actual replay outcomes. The curated live RAG suite should be rerun through the atomic critic, and retrieval should be improved until every case is measurable and mean recall@10 reaches the 0.80 promotion threshold.
